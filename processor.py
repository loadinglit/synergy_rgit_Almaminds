...
class YouTubeProcessor:
    def __init__(self, api_key: str, output_dir: str = "processed_videos"):
    ...
    def extract_highlights(
        ...
        try:
            ...
            custom_response = self.client.generate.text(
                video_id=video_id,
                prompt=key_moments_prompt
            )
            
            # Also get standard highlights as backup/additional content
            self.logger.info("Getting standard highlights...")
            standard_highlights = self.client.generate.summarize(video_id=video_id, type="highlight")
            ...
            try:
                ...
                self.logger.warning("Failed to parse JSON from custom prompt response, using standard highlights only")
            ...
            for i, moment in enumerate(key_moments):
                try:
                    ...
                    if end_time <= start_time or end_time - start_time < 1:
                        self.logger.warning(f"Invalid time range for moment: {description}")
                        continue
            ...
            if not highlights:
                self.logger.info("No custom key moments were successfully processed, using standard highlights")
                for i, highlight in enumerate(standard_highlights.highlights):
                    try:
                        safe_text = "".join(
                            c for c in highlight.highlight if c.isalnum() or c in " "
                        ).strip()[:30]
                        output_file = highlights_dir / f"highlight_{i+1}_{safe_text}.mp4"
                        ...
                        self.logger.info(f"Created standard highlight clip: {output_file}")
    ...
    def get_enhanced_keywords(self, video_id: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                self.logger.warning("Direct JSON parsing failed, attempting to extract JSON from response")
                ...
                try:
                    ...
                    if "seo_keywords" in text_response.lower():
                        keywords = [
                            word.strip(' "[](),.')
                            for word in text_response.split("keywords")[1].split("\n")[0].split(",")
                            if word.strip()
                        ]
                        default_response["seo_keywords"] = keywords[:5]  # Take up to 5 keywords
